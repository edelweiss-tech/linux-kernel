...
int smi_audio_init(struct pci_dev *pci)
{
	...
	strcpy(card->driver, "SM768 I2S");
	strcpy(card->shortname, "SM768 I2S");
	strcpy(card->longname, "SM768 I2S");

	snd_pcm_new(card,"falconi2s_pcm",0,1,0,&pcm);
	...
	strcpy(card->mixername, "SM768 Mixer Control");
