...
static void switch_to_mon_mod(struct pvt_hwmon *hwmon)
{
	...
	//ON PVT
	writel((BK_PVT_CTRL_EN_BIT) | ((hwmon->mon_mod) << 1),
	       hwmon->base + BK_PVT_CTRL);
}
...
static int temp2data(int temp)
{
	...
	/* Dont change the order of multiplication !!! */
...
static ssize_t pvt_show_voltage(struct device *dev,
...
static DEVICE_ATTR(name, 0444, pvt_show_name, NULL);
static DEVICE_ATTR(temp1_input, 0444, pvt_show_temp, NULL);
static DEVICE_ATTR(in1_input, 0444, pvt_show_voltage, NULL);
static DEVICE_ATTR(lvt_input, 0444, lvt_show, NULL);
static DEVICE_ATTR(hvt_input, 0444, hvt_show, NULL);
static DEVICE_ATTR(svt_input, 0444, svt_show, NULL);
static DEVICE_ATTR(temp1_min, 0644, pvt_show_temp_min, set_temp_min);
static DEVICE_ATTR(temp1_max, 0644, pvt_show_temp_max, set_temp_max);
static DEVICE_ATTR(in1_min, 0644, pvt_show_volt_min, set_volt_min);
static DEVICE_ATTR(in1_max, 0644, pvt_show_volt_max, set_volt_max);
static DEVICE_ATTR(mon_mod, 0644, pvt_show_mon_mod, set_mon_mod);
