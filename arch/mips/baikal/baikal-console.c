...
static __iomem void *uart_membase =
			(__iomem void *) KSEG1ADDR(EARLY_CONSOLE_BASE);

static inline void uart_write32(u32 val, unsigned int reg)
{
	writel(val, uart_membase + reg);
}

static inline u32 uart_read32(unsigned int reg)
{
