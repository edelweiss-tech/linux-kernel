...
#ifdef CONFIG_SYSFS
static ssize_t show_emc_stat(struct device *dev,
...
static DEVICE_ATTR(stat, 0644, show_emc_stat, NULL);

static ssize_t show_emc_rate(struct device *dev,
			     struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct be_emc *emc = platform_get_drvdata(pdev);
...
static DEVICE_ATTR(rate, 0644, show_emc_rate, NULL);
...
static irqreturn_t be_emc_irq(int irq, void *data)
{
	...
	if (irq == emc->irq_dfi) {
		dev_crit_ratelimited(emc->dev,
			"Parity or CRC error is detected on the DFI interface\n");
	...
	if (irq == emc->irq_ecr) {
		dev_warn_ratelimited(emc->dev,
				     "Correctable ECC error is detected\n");

		reg1 = readl(emc->regs + EMC_ECCCADDR0);
		reg2 = readl(emc->regs + EMC_ECCCADDR1);

		dev_warn_ratelimited(emc->dev,
			"RANK=%d ROW=%d BGRP=%d BANK=%d COL=%d\n",
			(reg1 >> 24) & 0x03, reg1 & 0x3ffff,
			(reg2 >> 24) & 0x03, (reg2 >> 16) & 0x07,
			reg2 & 0x0fff);
...
static void be_emc_init(struct be_emc *emc)
{
	...
	dev_info(emc->dev,
		"%s x%d ranks:%d ecc:%s burst:%d bus:DQ/%d timing:%dT bchop:%d bmode:%d\n",
		be_emc_ddr_type(info->type), info->cfg, info->ranks,
		info->ecc ? "on":"off", info->burst, info->dqdiv, info->timing,
		info->bchop, info->bmode);
