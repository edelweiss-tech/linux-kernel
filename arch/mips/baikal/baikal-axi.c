...
static DEVICE_ATTR(count, 0644, show_count, NULL);
...
static DEVICE_ATTR(addr, 0644, show_addr, NULL);
...
static DEVICE_ATTR(type, 0644, show_type, NULL);

static ssize_t show_test(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	return scnprintf(buf, PAGE_SIZE,
			 "Test interconnect error (0 - Slave internal error, 1 - No slave error)\n");
}
